  ...
  return (
    <div className="px-4 sm:px-6 py-5">
      <div className="flex items-center justify-between gap-3 mb-1">
        <div>
          <div className="text-[18px] font-bold text-text">About Organisation</div>
          <div className="text-[13.5px] text-text-muted mt-0.5">Portal → About Organisation</div>
        </div>
        <a
      ...
      <div className="rounded-2xl border border-primary/25 bg-primary-tint p-5 mt-4 mb-5">
        <div className="text-[21px] font-bold text-text">Aditi Tracking Support Pvt. Ltd.</div>
        <div className="flex items-center gap-2.5 flex-wrap mt-1.5">
          <span className="text-[14.5px] font-bold text-primary">🏆 India's Leading Telematics Company</span>
          <span className="w-1 h-1 rounded-full bg-text-muted" />
          <span className="text-[13.5px] text-text-muted">Est. 2011 · Mumbai, India</span>
        </div>
      </div>
...
function StatCard({ stat, onClick }) {
  return (
    ...
    >
      <div className="text-[28px] font-extrabold text-primary">{stat.value}</div>
      <div className="text-[12.5px] text-text-muted mt-1 uppercase tracking-wide">{stat.label}</div>
      {stat.link && <div className="text-[12.5px] font-bold text-primary mt-1.5">View All →</div>}
    </div>
  )
}
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-4 mb-4">
        <div className="rounded-2xl border border-border bg-surface p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[15.5px] font-bold text-text">Our Story</span>
          </div>
          <div className="text-[14.5px] text-text-muted leading-relaxed space-y-3">
            {OUR_STORY.map((p) => (
        ...
        <div className="rounded-2xl border border-border bg-surface p-5">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-[20px]">💎</span>
            <span className="text-[15.5px] font-bold text-text">Core Values</span>
          </div>
          <div className="flex flex-col gap-2.5">
            {CORE_VALUES.map((v) => (
              <div key={v.title} className="flex items-center gap-2.5">
                <span className="w-7 h-7 rounded-md bg-primary-tint text-primary flex items-center justify-center text-[15px] shrink-0">
                  {v.icon}
                </span>
                <div>
                  <div className="text-[14px] font-bold text-text">{v.title}</div>
                  <div className="text-[13px] text-text-muted">{v.desc}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="rounded-2xl border border-border bg-surface p-5">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-[20px]">🔭</span>
            <span className="text-[15.5px] font-bold text-text">Our Vision</span>
          </div>
          <p className="text-[14.5px] text-text-muted leading-relaxed">{VISION}</p>
        </div>
        <div className="rounded-2xl border border-border bg-surface p-5">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-[20px]">🎯</span>
            <span className="text-[15.5px] font-bold text-text">Our Mission</span>
          </div>
          <p className="text-[14.5px] text-text-muted leading-relaxed">{MISSION}</p>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-5">
        {LOCATIONS.map((loc) => (
          <div key={loc.name} className="rounded-2xl border border-border bg-surface p-5 relative overflow-hidden">
            ...
            <div className="flex items-center gap-2 mb-3">
              <span className="text-[20px]">{loc.icon}</span>
              <span className="text-[15px] font-bold text-text flex-1">{loc.name}</span>
              <span
            ...
            <div className="text-[14px] text-text-muted leading-relaxed mb-3">
              {loc.lines.map((l) => (
                <div key={l}>{l}</div>
              ))}
            </div>
            {loc.phone && (
              <div className="text-[14px] text-primary mb-3">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="block text-center text-[13.5px] font-semibold rounded-md border px-3 py-2"
      ...
      <div className="rounded-2xl border border-border bg-surface p-5">
        <div className="flex items-center gap-2 mb-3">
          <span className="text-[20px]">📡</span>
          <span className="text-[15.5px] font-bold text-text">Upcoming Office</span>
        </div>
        <div className="flex gap-2.5 flex-wrap">
          {UPCOMING_OFFICES.map((city) => (
            <div key={city} className="rounded-lg bg-surface-2 border border-border px-4 py-2.5 text-[14.5px] font-bold text-text">
              {city}
            </div>
          ))}
        </div>
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface p-5">
      <div className="flex items-center gap-2 mb-4">
        <span className="text-[20px]">🏆</span>
        <span className="text-[15.5px] font-bold text-text">Company Milestones — 15-Years Journey</span>
      </div>
      <div className="flex flex-col gap-4">
        {MILESTONES.map((m) => (
          <div key={m.year} className="flex gap-3.5">
            <div className="w-12 shrink-0 text-[15px] font-extrabold text-primary">{m.year}</div>
            <div className="w-2 h-2 rounded-full border-2 border-primary mt-1 shrink-0" />
            <div className="pb-1">
              <div className="text-[14.5px] font-bold text-text">{m.title}</div>
              <div className="text-[13.5px] text-text-muted mt-0.5 leading-relaxed">{m.desc}</div>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface p-5" style={{ borderLeft: `4px solid ${cert.accent}` }}>
      <div className="flex items-start gap-3 mb-3">
        ...
        <div>
          <div className="text-[12.5px] uppercase tracking-wide text-text-muted font-bold mb-0.5">{cert.category}</div>
          <div className="text-[16px] font-extrabold text-text">{cert.title}</div>
        </div>
      </div>
      ...
      <div
        className="mt-2.5 inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[12.5px] font-bold"
        style={{
  ...
  return (
    <div>
      <div className="rounded-xl border border-primary/20 bg-primary-tint p-4 mb-5 flex items-center gap-3.5">
        ...
        <div>
          <div className="text-[15px] font-bold text-text mb-0.5">Certifications &amp; Accreditations</div>
      ...
      <div className="mt-4 rounded-xl border border-border bg-surface p-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="text-[14.5px] font-bold text-text">🏆 Certified Excellence Since 2019</div>
        ...
        <div className="flex gap-2 flex-wrap">
          {CERT_BADGES.map((b) => (
            <span key={b} className="text-[13px] font-bold text-primary bg-primary-tint border border-primary/20 rounded-full px-3 py-1">
              {b}
            </span>
          ))}
        </div>
